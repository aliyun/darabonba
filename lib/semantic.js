...
class TypeChecker {
  ...
  checkImports(ast) {
    ...
    for (let i = 0; i < ast.imports.length; i++) {
      ...
      let realSpecPath;
      if (path.isAbsolute(specPath) || specPath.startsWith('./') || specPath.startsWith('../')) {
        if (specPath.endsWith('.dara') || specPath.endsWith('.tea') || specPath.endsWith('.spec')) {
          realSpecPath = path.resolve(pkgDir, specPath);
        } else {
          const libMetaPath = getDarafile(path.resolve(pkgDir, specPath));
          const libMeta = JSON.parse(fs.readFileSync(libMetaPath, 'utf-8'));
          realSpecPath = path.join(path.resolve(pkgDir, specPath), libMeta.main);
        }
      } else {
        const key = `${specPath}`;
