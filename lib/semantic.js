...
class TypeChecker {
  ...
  getNotes(ast) {
    let lastToken = 0;
    ast.moduleBody.nodes.forEach((item) => {
      item.notes = note.getNotes(ast.notes, lastToken, item.tokenRange[0]);
      lastToken = item.tokenRange[1];
    });
  }
  ...
  getType(t, extern) {
    ...
    if (t.type === 'iterator' || t.type === 'asyncIterator') {
      ...
      return {
        type: t.type,
        valueType: this.getType(t.valueType, extern)
      };
    }
