...
const { isBasicType } = require('./util');
...
function isAssignable(expected, actual, expr) {
  ...
  if (expected.type === 'map' && actual.type === 'map') {
    if (expr.type === 'object' && expr.fields.length === 0) {
      return true;
    }
  ...
  if (expected.type === 'array' && actual.type === 'array') {
    ...
    if (expr.type === 'array' && expr.items.length === 0) {
      return true;
    }
  }
  ...
  if (expected.type === 'basic' && actual.type === 'basic') {
    // any = other type
  ...
  if (expected.type === 'basic' && expected.name === 'number') {
    // number = int32 should ok
    if (actual.type === 'basic' && actual.name === 'int32') {
      return true;
    }
  }
...
class TypeChecker {
  ...
  checkImports(ast) {
    ...
    const pkgPath = path.join(pkgDir, 'Teafile');
    if (!fs.existsSync(pkgPath)) {
      this.error(`the Teafile not exists`);
    }
    ...
    for (let i = 0; i < ast.imports.length; i++) {
      ...
      if (!pkg.libraries[aliasId] && pkg.name !== aliasId) {
        this.error(`the import "${aliasId}" not defined in Teafile`, item);
      }
      ...
      if (specPath.startsWith('./') || specPath.startsWith('../')) {
        if (specPath.endsWith('.tea') || specPath.endsWith('.spec')) {
          realSpecPath = path.join(pkgDir, specPath);
        } else {
          const libMetaPath = path.join(pkgDir, specPath, 'Teafile');
        ...
        if (!this.libraries[key]) {
          this.error(`the module id "${aliasId}" has not installed, use \`tea install\` first`, item);
        }

        const libPath = path.join(this.root, this.libraries[key]);
        const libMetaPath = path.join(libPath, 'Teafile');
        const libMeta = JSON.parse(fs.readFileSync(libMetaPath, 'utf-8'));
  ...
  visitAPI(ast) {
    ...
    const preEnv = env.local;
    env.local = new Env(preEnv);
  ...
  isNumberType(expr, env) {
    const type = this.getExprType(expr, env);
    return type.type === 'basic' && type.name === 'number';
  }
  ...
  getFieldType(find, modelName, moduleName) {
    ...
    if (find.fieldValue.fieldType === 'array') {
      if (find.fieldValue.fieldItemType.tag === Tag.TYPE) {
        return {
          type: 'array',
          itemType: _basic(find.fieldValue.fieldItemType.lexeme)
        };
      }

      return {
        type: 'array',
        itemType: _model(find.fieldValue.fieldItemType.lexeme, moduleName)
      };
    }
    ...
    for (var i = 0; i < modelBody.nodes.length; i++) {
      ...
      if (fieldValue.type === 'modelBody') {
        ...
        if (fieldValue.fieldType === 'array') {
          if (fieldValue.fieldItemType.tag === Tag.ID) {
            const typeId = fieldValue.fieldItemType.lexeme;
            const type = this.getIdType(typeId);
            if (!type) {
              this.error(`the type "${typeId}" is undefined`, fieldValue.fieldItemType);
            }
            fieldValue.fieldItemType.idType = type;
          } else if (fieldValue.fieldItemType.tag === Tag.TYPE) {
            // TODO
          } else if (fieldValue.fieldItemType.type === 'map') {
            // TODO
          } else {
            const submodel = `${modelName}.${fieldName}`;
            this.flatModel(root, node.fieldValue.fieldItemType, submodel);
