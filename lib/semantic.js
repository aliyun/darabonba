...
function isAssignable(expected, actual, expr) {
  ...
  if (isNumber(expected.name) && isNumber(actual.name)) {
    if (expected.name === 'number') {
      return true;
    }

    if (isInteger(expected.name) && actual.name === 'integer') {
      return true;
    }

    if ((expected.name === 'long' || expected.name === 'ulong') && isInteger(actual.name)) {
      return true;
    }

    if (expected.name === 'ulong' && actual.name === 'long') {
      return true;
    }
  }
...
class TypeChecker {
  ...
  getFieldType(find, modelName, moduleName) {
    ...
    if (find.fieldValue.fieldType === 'array') {
      ...
      // for filed: [{}]
      if (find.fieldValue.fieldItemType.type === 'modelBody') {
        return {
          type: 'array',
          itemType: _model(find.fieldValue.itemType, moduleName)
        };
      }
